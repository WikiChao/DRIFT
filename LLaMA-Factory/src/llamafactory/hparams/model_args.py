...
class BaseModelArguments:
    ...
    # ---- delta prior options ----
    delta_prior1_ckpt: Optional[str] = field(
        default="deepseek-ai/DeepSeek-R1-Distill-Qwen-7B",
        metadata={"help": "Primary delta-prior checkpoint (e.g., deepseek-ai/DeepSeek-R1-Distill-Qwen-7B)."},
    )
    delta_prior2_ckpt: Optional[str] = field(
        default=None,
        metadata={"help": "Secondary delta-prior checkpoint (optional)."},
    )
    delta_prior_only_layers: Optional[str] = field(
        default=None,
        metadata={
            "help": "Comma-separated list of layer indices to apply delta prior on, e.g. '0,1,2'. Leave empty for all."
        },
    )
    delta_prior_alpha: Optional[float] = field(
        default=None,
        metadata={"help": "Alpha coefficient for delta prior blending (None uses implementation default)."},
    )
    delta_prior_skip_first_n: int = field(
        default=0,
        metadata={"help": "Skip applying delta prior to the first N transformer layers."},
    )
    delta_prior_normalize_delta_to_param: bool = field(
        default=True,
        metadata={"help": "Normalize delta by parameter magnitude before applying."},
    )
    delta_prior_include_bias: bool = field(
        default=True,
        metadata={"help": "Whether to include bias parameters in delta prior."},
    )
    delta_prior_scale: str = field(
        default="adaptive_cosine",
        metadata={"help": "Scaling strategy for delta prior (e.g., 'adaptive_cosine')."},
    )
    delta_prior_merge_patterns: Optional[Union[dict, str]] = field(
        default=None,
        metadata={
            "help": "Merge patterns for delta prior application. Accepts dict or JSON string."
        },
    )
    # ---- end delta prior options ----
    ...
    def __post_init__(self):
        ...
        # Parse JSON for delta_prior_merge_patterns if provided as string
        if isinstance(self.delta_prior_merge_patterns, str) and self.delta_prior_merge_patterns.startswith("{"):
            self.delta_prior_merge_patterns = _convert_str_dict(json.loads(self.delta_prior_merge_patterns))
